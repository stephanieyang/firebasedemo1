	...
	<script>
		...
		this.submitInfo = function(event) {
			...
			var recipeData = {
				...
				"category":category,
				"imageLink":imageLink,
				//"ingredients":ingredients, // store in different push
				"directions":directions,
			};

			// two subcategories in the recipe data for easy filtering: by user and by category
			var userRecipeRef = dataRef.child("recipeData/user/" + user);
			var categoryRecipeRef = dataRef.child("recipeData/category/" + category);

			// first, push info to user data
			userRecipeRef.push(recipeData, function(err) {
				if(err) {
					console.log("Error: " + err);
				}
			}).then(function(result) {
				userIngredRef = dataRef.child("recipeData/user/" + user + "/" + result.key + "/ingredients");
				categoryIngredRef = dataRef.child("recipeData/category/" + category + "/" + result.key + "/ingredients");
				//console.log(result.key);
				for(var i = 0; i < ingredients.length; i++) {
					userIngredRef.push(ingredients[i], function(err) {
						console.log("Error: " + err);
					});
				}
			});
